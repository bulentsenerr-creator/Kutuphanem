// Barkod Tarama (ZXing) - Pro Max Uyumlu, Çoklu CDN + sağlam getUserMedia
...
function cameraErrorMessage(err){
  ...
  if(/Content Security Policy|CSP|Refused to load the script|violates the following Content Security Policy/i.test(msg)){
    return 'ZXing yüklenemedi: CSP/CDN engeli olabilir. (Yerel ZXing dosyası ekleyin)';
  }
...
async function openScanner(){
  ...
  try{
    await loadZXing();
    // warm-up permission (Android/iOS)
    try{
    ...
    $('scanInfo').textContent = cameraErrorMessage(e) + ` (${e?.name||'Error'})`;
    console.error(e);
  }
}
